...
function hashToModuleId(hash, seed, hashSize) {
  ...
  // (nibbles) long. double the hash to allow overflow
  hash = hash + hash;
...
WebpackStableModuleIdAndHash.prototype.apply = function (compiler) {
  ...
  if (hashSize > 53) {
    // In JavaScript, only integers up to 2^53 (exclusive) can be considered safe,
  ...
  // Generate module id by md5 value of file path. Since webpack 1.x can not use a
  // non-number as a module id, convert the md5 (hex) to unique number. spooned
  // from
  // https://github.com/webpack/webpack/blob/master/lib/HashedModuleIdsPlugin.js
  compiler
    .plugin("compilation", function (compilation) {
      compilation
        .plugin("before-module-ids", function (modules) {
          modules
            .forEach(function (module) {
              if (module.libIdent && module.id === null) {
                var modulePath = module.libIdent({context: context});
