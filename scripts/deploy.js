...
const pushDist = () => {
  const commands1 = [
    `cd ../../${config.repo.name}`, 'git pull',
    //    'git ls-files --deleted -z | xargs -0 git rm',
    'git add .',
    `git commit -m "${config.commitMessage}"`
  ]
  // Commit will throw an error if we commit no files. The push is therfore broken
  // into another series of commands where it can be run on 'then' or 'catch'.
  const commands2 = [`cd ../../${config.repo.name}`, 'git push']
  return exec(joinCommands(commands1)).then(() => exec(joinCommands(commands2))).catch(() => exec(joinCommands(commands2)))
}

const copyDist = () => {
  // Create the destination folder if it doesn't exist
  const destination = `../../${config.repo.name}/client`
  if (!fs.existsSync(destination)) {
    fs.mkdirSync(destination)
  }
  return fs.copyAsync('./build/client', destination)
}
